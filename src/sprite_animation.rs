...
fn setup(settings: Arc<LiveSettings>, mut commands: Commands, asset_server: Res<AssetServer>, mut texture_atlas_layouts: ResMut<Assets<TextureAtlasLayout>>) {
	commands.spawn(Camera2d);

	// Create a minimal UI explaining how to interact with the example
	let config = exit_on_error(settings.config());
	commands.spawn((
		Text::new(format!("Hello {}!\n{Left Arrow: Animate Left Sprite\nRight Arrow: Animate Right Sprite", config.example_greet)),
		Node {
