...
def choix_carte_greedy(plateau: Grid, dico_main: dict[int, int], dico_options: dict[str, int], joueuse_active: int) -> tuple[int, int, int]:
	...
	available: list[Pos] = [Pos(L, C) for L in range(n) for C in range(n) if emplacement_jouable(plateau, L, C)]
	...
	for i, pos in enumerate(moves):
		if pos is None:
			continue
		stack = stacks[i]
		entries: list[tuple[int, int]] = [(card_val, _score_delta(stack, card_val)) for card_val in range(maxC + 1) if hand.get(card_val, 0) > 0]
	...
	for i, candidates in enumerate(scored_moves):
		...
		if best_delta is None or delta > best_delta or (delta == best_delta and card_val < best_card):
			best_delta = delta
			best_card = card_val
			best_pos = moves[i]
